...
const Notifications = ({ notifications, removeNotification, clearNotifications }) => {
    let notificationsCmp = notifications
        .map((n)=><Notification key={n.id} id={n.id} data={n.payload} type={n.type} onRemove={removeNotification} isNew={n.isNew} />)
    const clear = ()=>{
        setTimeout(()=>{
            clearNotifications()
        }, 5000)}
